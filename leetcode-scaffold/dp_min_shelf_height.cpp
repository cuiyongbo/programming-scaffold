...
We want to place these books in order onto bookcase shelves whose total width is shelf_width.
...
{ // dp solution
    ...
    // dp[j] = min(dp[i-1]+max(h[i:j])) for j in [i, n] if sum(w[i:j])<=shelf_width, Note that books[i:j] are put onto the same level
    int n = books.size();
    ...
    for (int i=0; i<n; ++i) {
        ...
        // put books[i:j] onto the same level of the shelf
        for (int j=i; j<n; ++j) {
            w += books[j][0];
            if (w > shelf_width) { // we cannot change the order of books in the given sequence
                break;
            }
            h = max(h, books[j][1]);
...
int main() {
    ...
    minHeightShelves_scaffold("[[1,3],[2,4],[3,2]]", 6, 4);
