...
std::vector<int> Solution::findFrequentTreeSum(TreeNode* root) {
    ...
    // return the sum of subtree rooted at node, traversing the tree in post-order way
    ...
    dfs(root);
    // elements in ans are sorted in ascending order
    for (auto it: mp) {
...
int Solution::minCameraCover(TreeNode* root) {
    ...
    int ans = 0;
    // return the State of node, traversing the subtree rooted in node in post-order way
    std::function<State(TreeNode*)> dfs = [&] (TreeNode* node) {
        ...
        if (l==State::Camera || r==State::Camera) {
            ...
            ans++;
            return State::Camera;
        } else { // l==State::Covered && r==State::Covered 
            return State::None;
        }
    };
...
balance(node) = abs(num_node - num_coin)
*/
int Solution::distributeCoins(TreeNode* root) {
    int ans = 0;
    using element_t = std::pair<int, int>; // number of nodes of the subtree, number of coins of the subtree
    // traverse the subtree rooted at node in post-order way
    std::function<element_t(TreeNode*)> dfs = [&] (TreeNode* node) {
        ...
        auto l = dfs(node->left);
        auto r = dfs(node->right);
        ans += abs(l.first - l.second); // balance of left subtree
        ans += abs(r.first - r.second); // balance of right subtree
