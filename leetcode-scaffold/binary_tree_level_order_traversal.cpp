...
int Solution::widthOfBinaryTree(TreeNode* root) {
    ...
    int width = 0;
    // perform bfs traversal to find the width of each level
    // for a node with node_id i (0-indexed), left(i) = 2*i+1, right(i) = 2*i+2
    using element_t = std::pair<TreeNode*, int>; // node, node_id
    std::queue<element_t> q; q.push({root, 0}); // first-in, first-out
    while (!q.empty()) {
        int offset = 0; // minus the offset to avoid integer overflow when calculating node_id
        int left=-1, right=-1;
...
int Solution::deepestLeavesSum(TreeNode* root) {
    ...
    int ans = 0; // the sum of nodes' value in the last traversal
