...
int Solution::rob_198(vector<int>& moneys) {
    // dp[i] means the maximum money a thief can rob with house[:i], i is inclusive
...
int Solution::rob_213(vector<int>& moneys) {
    ...
    // return the maximum money a thief can rob with house[l:r], r is not inclusive
...
But you must also delete every element equal to nums[i]-1 or nums[i]+1. You start with 0 points. Return the maximum number of points you can earn by applying such operations.
Example 1:
    Input: nums = [3, 4, 2]
    Output: 6
...
int Solution::deleteAndEarn(vector<int>& nums) {
    ...
    int l = *(range.first);
    int r = *(range.second);
    vector<int> points(r-l+1, 0); // it is a count map of nums, where its indices are elements of nums
    for (auto n: nums) {
        points[n-l] += n;
    }
...
int Solution::minSwap(vector<int>& A, vector<int>& B) {
    ...
    for (int i=1; i<n; i++) {
        // 1. both A[i-1:i] and B[i-1:i] are already sorted
        // we may swap either at both positions i-1 and i or neither positions
        if (A[i-1] < A[i] && B[i-1] < B[i]) { 
            keep[i] = keep[i-1]; // no need to perform swap
            exchange[i] = exchange[i-1]+1; // do swap at both postitions i-1 and i
        }
