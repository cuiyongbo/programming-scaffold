...
def do_bge_large_zh_ensemble_inference(client, model_name: str, inputs: list, verbose=False):
    # inputs = ["hello world", "nice to meet you"]
    inputs = [s.encode() if isinstance(s, bytes) else s for s in inputs]
    in0 = np.array([inputs], dtype=np.object_).reshape(-1, 1)
    input_tensors = [
    ...
    output_tensors = [
        grpcclient.InferRequestedOutput("ensemble_dense_vecs"),
        grpcclient.InferRequestedOutput("ensemble_dense_token_num"),
    ]
    infer_rsp = client.infer(model_name, inputs=input_tensors, outputs=output_tensors, timeout=client_timeout_in_second)
    if verbose:
        dense_vecs = infer_rsp.as_numpy("ensemble_dense_vecs")
        token_num = infer_rsp.as_numpy("ensemble_dense_token_num")
        print(f"dense_vecs: {dense_vecs.shape}, token_num: {token_num}")
...
def do_inference(client, model_name: str, inputs: list, verbose=False):
    ...
    elif model_name in ["bge_large_zh_ensemble"]:
        return do_bge_large_zh_ensemble_inference(client, model_name, inputs, verbose)
...
parser.add_argument("-m", "--model", type=str, default="bge_large_zh_ensemble", choices=["bert", "python_bge_m3_onnx", "python_bge_large_zh_onnx", "bge_large_zh_ensemble"])
